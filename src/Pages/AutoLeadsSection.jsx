...
export default function AutoLeadsSection({ email }) {
  ...
  // Save lead changes to the backend
  const saveLeadChanges = async (leadId, updates) => {
    try {
      const params = new URLSearchParams();
      params.append('updateLead', 'true');
      ...
      console.log('Successfully updated lead:', leadId, updates);
    } catch (error) {
  ...
  const debouncedSave = debounce(saveLeadChanges, 1000);
    ...
    setLeads(prevLeads => {
      return prevLeads.map(lead => {
        if (lead['Lead ID'] === leadId) {
          ...
          debouncedSave(leadId, { [key]: updatedLead[key] });
                ...
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg border">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="text-gray-800">{lead["Name"] || '—'}</div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="text-gray-800">{lead["City"] || '—'}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="text-blue-700 font-medium">{lead["Project"] || '—'}</div>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-500 mb-1">Source</label>
                          <div className="text-gray-800">{lead["Source"] || '—'}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg border">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-500 mb-1">Called?</label>
                          <select 
                            value={lead["Called?"] || ""} 
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-500 mb-1">Lead Quality</label>
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              lead["Lead Quality"] === "Hot" ? "bg-red-100 text-red-800" :
                              lead["Lead Quality"] === "Warm" ? "bg-yellow-100 text-yellow-800" :
                              lead["Lead Quality"] === "Cold" ? "bg-blue-100 text-blue-800" :
                              "bg-gray-100 text-gray-800"
                            }`}>
                              {lead["Lead Quality"] || 'Not Rated'}
                            </span>
                          </div>
                        </div>
                      </div>
